...
- **Resource nhận theo TÊN, tool nhận PATH — hệ quả khi alias mất:** tool (`detect_changes` · `query` · `context` ·
  `impact` · `cypher`) vẫn chạy với path đầy đủ; nhưng **resource `gitnexus://repo/<name>/clusters|processes|context`
  CHẾT** (`repo not found`). Muốn clusters/processes → **khôi phục alias TRƯỚC**, đừng lặng lẽ thay bằng git-tree/doc.
- **SỰ THẬT VỀ NGUỒN — no silent source-swap (BẮT BUỘC).** Chỉ nói "clusters/processes/luồng GitNexus" khi
  resource/tool **thực sự trả về**. Nếu nó gọi hụt mà bạn dựng hình từ **git-tree/grep/doc** → ghi ĐÚNG nguồn đó,
  **TUYỆT ĐỐI KHÔNG dán nhãn GitNexus**. Mỗi artifact (sơ đồ · bảng phạm vi · finding) mang nguồn thật của nó.
...
2. list_repos({})                        → lấy đúng path index; có PDG chưa; embeddings/stats
2b. THỬ resource theo TÊN: đọc gitnexus://repo/<repo>/context
    → "repo not found" = ALIAS MẤT → resource (clusters/processes) KHÔNG dùng được (chỉ tool-nhận-PATH chạy).
       Quyết NGAY: (a) khôi phục alias `node .gitnexus/run.cjs analyze --name <repo>` để có clusters/processes,
       HOẶC (b) đánh dấu Tầng-1 nguồn = git-tree/doc và KHÔNG gán nhãn "GitNexus" cho sơ đồ (xem quy tắc nguồn).
...
- **Sơ đồ 1 — Kiến trúc & vùng đụng** (`flowchart`): các tầng/module từ `clusters` (nếu resource CHẠY) + hướng
  phụ thuộc; **tô đậm/đánh dấu** node/cạnh mà diff chạm vào. ⚠️ `clusters` gọi hụt (alias mất) → dựng từ git-tree/doc
  và **ghi rõ nguồn = git-tree/doc, KHÔNG ghi "clusters GitNexus"** (no silent source-swap).
...
### Nguồn GitNexus thực dùng (minh bạch — BẮT BUỘC, chống overclaim)
| Tool/Resource | Chạy? | Đóng góp thật vào kết luận |
|---|---|---|
| detect_changes | ✅/⚠️/❌ | <vd: 2.874 symbol · 106 flow · risk cơ học → bảng Phạm vi> |
| query (trùng lặp) | ✅/❌ | <candidate tìm được / không> |
| clusters·processes (resource) | ✅/❌ | <nếu ❌: sơ đồ dựng từ git-tree/doc — KHÔNG phải GitNexus> |
| impact·context·cypher·explain | ✅/❌ | <định vị / PDG / …> |
- Ghi thẳng cái **gọi hụt / không dùng**. Evidence đến từ git diff/grep/doc → nói RÕ, đừng gán GitNexus.
```
...
- [ ] **Nguồn GitNexus thực dùng** (bắt buộc): ghi tool nào chạy/đóng góp gì · cái gọi hụt · sơ đồ dựng từ git-tree/doc thì KHÔNG gán GitNexus (no silent source-swap)
